  ...
  return (
    ...
    >
      <img
        src={track.albumUrl}
        alt="album photo"
        className="w-full h-full object-cover absolute inset-0 rounded-[50px] opacity-80 group-hover:opacity-100"
      />

      <div className="absolute bottom-10 inset-x-0 ml-4 flex items-center space-x-3.5">
        <div className="h-10 w-10 bg-[#15883e] rounded-full flex items-center justify-center group-hover:bg-[#1bb954] flex-shrink-0">
        ...
        <div className="text-[15px]">
            <h4 className="truncate w-44 font-extrabold">{track.title}</h4>
            <h6>{track.artist}</h6>
        </div>
      </div>
    </div>
